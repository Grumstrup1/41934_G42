...
-	Really useful to compare different options for the same element.
-	Can be added for price analysis.

Group 48: 
-	Great that it’s possible to adjust your inputs while the code is running, and that you don’t have to load/run the script every time you want to make an adjustment.
-	Nicely formatted menu when using the code, nice ability to update prices while using the tool.
-	The base area calculations seem to be set up robustly, which is cool. Prioritization is awesome. The fact that the base input is only an ARCH .Ifc model is awesome too!
-	Love the menu!
-	It would be nice to export the data in a way, to use it later, and to document it.

Group 49: 
-	The menu provides a nice overview of the functions of the tool. 
-	Customizability is nice, which shows flexibility to edit for different types of parameters. 
-	Good sanity check – for budgeting, and quick way to mock op tools for a rapid overview. 

### did the tool address the use case you identified?

### What stage does the tool you created work in Advanced Building Design? ( Stage A, B, C and/ or D).
Mainly Stage B (Early Design).
As soon as you have a basic IFC model, the tool gives a clear overview and helps compare scenarios. It’s great for early decision-making, especially if the budget is a concern.

It can also be used later for comparisons, but it’s not intended for detailed final validation; it’s more of a rapid estimation and overview tool.
